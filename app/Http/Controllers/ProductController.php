...
class ProductController extends Controller
{
  ...
  public function index(
    ...
    $allProductLines = Cache::remember(
      ...
      function () {
        return ProductLine::with(['productSubcategory', 'printMethod'])->get();
      }
    );
    ...
    $swatchesProduct = $this->getSwatchesProduct($productLine, $activeArray);
  ...
  private function getFeatures($productLineId, $activeArray)
  {
    ...
    $allProductFeatures = Cache::remember(
      ...
      new DateInterval('P1M'),
      function () {
        return ProductFeature::with(['productFeaturesPivot', 'productLines'])
          ->orderBy('id', 'desc')
          ->get();
      }
    );
    ...
    $narrowedProductFeatures->each(function (ProductFeature $featureItem) use (
  ...
  private function getTextNotes($productLine, $activeArray)
  {
    ...
    $expandedProductLine = $productLine
      ->load([
        'productNotes' => function ($query) use ($activeArray) {
          $query->whereIn('active', $activeArray)->orderBy('priority', 'asc');
        }
      ])
      ->load([
    ...
    return $numNotes > 0 ? $this->formatTextNotes($expandedProductLine) : null;
  }
  ...
  private function getProducts($productLine, array $activeArray)
  {
    ...
    $perThousand = "";
    if (
      $productLine->productSubcategory->product_category_id === "napkin" &&
    ...
    $filteredPLQBs = $allPrices->filter(function ($value) use ($activeArray) {
      return in_array($value->productLineQuantityBreak->active, $activeArray);
    });
    ...
    foreach ($products as $product) {
      ...
      $output .=
        "<div class='{$singletonClass} item-info card text-center'>" . PHP_EOL;
      ...
      $output .=
        "<div class='thumbnail__overlay {$rotatorClass} rounded'>" . PHP_EOL;
      ...
      foreach ($productLine->productLineQuantityBreaks as $quantityBreak) {
      ...
      foreach ($chargeNames as $chargeName) {
        ...
        $output .=
          "<img src='{$iconHeader}' class='svg icon-screen'>" . PHP_EOL;
  ...
  private function getThumbnails($productLine, $product, $decodedProductName)
  {
    ...
    foreach ($productList as $image) {
      ...
      $result .=
        "<div class='{$rotatingImagesClass}' {$rotatingImagesStyle}>" . PHP_EOL;
      $result .=
  ...
  private function getSwatchesImprintColor($productLine, array $activeArray)
  {
    ...
    $colorTypes = Cache::remember(
      ...
      new DateInterval('P1M'),
      function () use ($productLine, $activeArray) {
        return ColorType::whereHas('productLines', function ($query) use (
          $activeArray,
          $productLine
        ) {
          $query
              ...
              $query
                ->with([
                  'printMethods' => function ($subquery) use (
                    ...
                    $subquery
                      ->where('print_method_id', $productLine->print_method_id)
                      ->whereIn('color_print_method.active', $activeArray)
                      ->orderBy('priority', 'asc');
                  }
                ])
                ->whereIn('active', $activeArray);
  ...
  private function getSwatchesImprintType($productLine, array $activeArray)
  {
    ...
    $imprintTypes = Cache::remember(
      ...
      new DateInterval('P1M'),
      function () use ($productLine, $activeArray) {
        return ImprintType::whereHas('productLines', function ($query) use (
          $activeArray,
          $productLine
        ) {
          $query
    ...
    $output .= "<div class='d-flex flex-row'>" . PHP_EOL;
    $output .=
      "<ul class='swatches__list list-unstyled d-flex flex-wrap'>" . PHP_EOL;
